...
let lists_add list var =  
  ...
  let detect_string_type_or_other_and_return_nwe_variables _string _variables _dict = 
    if String.length _string >= 2 && String.get _string 0 = '"' && String.get _string ((String.length _string)-1) = '"' then
      ...
      let new_variables = removed_last_elm @ [lists_add last_elm (remove_string_quote _string)] in          
      new_variables
    else
  ...
  let rec variable_list_to_list variable_list lists idx =
    if List.length variable_list = idx then
      ...
      in 
      variable_list_to_list variable_list (lists@[_nth_elm]) (idx+1)
  ...
  let evalute_function _method_name _args = 
    if _method_name = "join" then
      ...
      | [Variable _sep; Lists extracted_list] ->
        let string_list =  variable_list_to_list extracted_list [] 0 in
  ...
  let rec parse_and_evalute original_content bit_string args variables char_idx array_flag = 
    ...
    let idx_char = String.get original_content char_idx in 
